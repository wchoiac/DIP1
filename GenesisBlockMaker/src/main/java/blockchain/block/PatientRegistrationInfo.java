...
public class PatientRegistrationInfo implements Raw {
    ...
    public PatientRegistrationInfo(long timestamp, ECPublicKey patientPublicKey, byte[] encryptedInfo, byte[] signature)
    {
    ...
    public static PatientRegistrationInfo parse(ByteArrayReader byteArrayReader) throws BlockChainObjectParsingException {
        PatientRegistrationInfo patientRegistrationInfo = new PatientRegistrationInfo();

        patientRegistrationInfo.setTimestamp(GeneralHelper.bytesToLong(byteArrayReader.readBytes(Long.BYTES)));

        int encryptedInfoLength = GeneralHelper.bytesToInt(byteArrayReader.readBytes(Integer.BYTES));
        patientRegistrationInfo.setEncryptedInfo(byteArrayReader.readBytes(encryptedInfoLength));
        try {
            patientRegistrationInfo.setPublicKey(SecurityHelper.getECPublicKeyFromCompressedRaw(byteArrayReader.readBytes(Configuration.RAW_PUBLICKEY_LENGTH),Configuration.ELIPTIC_CURVE));
        } catch (InvalidKeySpecException e) {
            e.printStackTrace();
            throw new BlockChainObjectParsingException();
        }
        patientRegistrationInfo.setSignature(byteArrayReader.readBytes(Configuration.SIGNATURE_LENGTH));
    ...
    public boolean equals(Object o) {
        ...
        PatientRegistrationInfo that = (PatientRegistrationInfo) o;
