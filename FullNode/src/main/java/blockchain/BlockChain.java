...
public class BlockChain {
    ...
    public int checkHeaders(BlockHeader[] headers) throws IOException, BlockChainObjectParsingException {
        ...
        for (BlockHeader header : headers) {
            ...
            try {
                if (!SecurityHelper.verifyRawECDSASignatureWithContent(validatorInfoForInternal.getAuthorityInfo().getPublicKey(), header.getSignatureCoverage()
                        , header.getValidatorSignature(),Configuration.BLOCKCHAIN_HASH_ALGORITHM,Configuration.ELIPTIC_CURVE))
                    return -1;
    ...
    public boolean checkNextBlock(Block block, ArrayList<Transaction> transactionPool) throws BlockChainObjectParsingException, IOException, FileCorruptionException {
        ...
        if (block.getHeader().getVote() != null && !checkVote(validatorInfo.getAuthorityInfo().getIdentifier(), block.getHeader().getVote()))
            return false;
    ...
    public AuthorityInfo addBlock(Block block, ArrayList<Transaction> transactionPool) throws IOException, BlockChainObjectParsingException {
        ...
        if (block.getHeader().getVote() != null) {
            processedAuthority = processVote(block.getHeader().getValidatorIdentifier(), block.getHeader().getVote());
        }
        ...
        if (block.getContent().getMedicalOrgRevocationList() != null)
            processRevocation(block.getContent().getMedicalOrgRevocationList());
        if (block.getContent().getTransactions() != null)
    ...
    private void processRevocation(byte[][] revokedIdentifiers) {
        for (byte[] info : revokedIdentifiers) {
            cachedCurrentMedicalOrgList.remove(info);
        }
    }
    private AuthorityInfo processVote(byte[] voterIdentifier, Vote vote) {
        ...
        if (changedAuthorityVoting.getNumAgree() >= getValidationInterval()) {
            if (changedAuthorityVoting.isAdd()) {
            ...
            votingToBeRemoved = changedAuthorityVoting;
        } else if (changedAuthorityVoting.getNumDisagree() >= getValidationInterval()) {
            votingToBeRemoved = changedAuthorityVoting;
        }

        if (votingToBeRemoved != null) {
            currentVotingList.remove(votingToBeRemoved);
            return votingToBeRemoved.getBeneficiary();
        }
    ...
    private void removeFromAuthorizationList(MedicalOrgInfo[] authorizedMedicalOrgInfos, ArrayList<MedicalOrgInfo> authorizationList) {
        for (MedicalOrgInfo medicalOrgInfo : authorizedMedicalOrgInfos) {
            authorizationList.remove(medicalOrgInfo);
        }
    }

    private void removeFromRevocationList(MedicalOrgInfo[] revokedMedicalOrgInfos, ArrayList<MedicalOrgInfo> revocationList) {
        for (MedicalOrgInfo medicalOrgInfo : revokedMedicalOrgInfos) {
            revocationList.remove(medicalOrgInfo);
        }
    }

    private void removeFromRegistrationList(PatientInfo[] patientInfos, ArrayList<PatientInfo> registrationList) {
        for (PatientInfo patientInfo : patientInfos) {
            registrationList.remove(patientInfo);
        }
    }
    ...
    public void initializeChainWithBestChain(ArrayList<Transaction> transactionPool) throws Exception {
    ...
    public void loadCurrentBest(ArrayList<Transaction> transactionPool) throws IOException, BlockChainObjectParsingException, FileCorruptionException {
        ...
        if (isNextBlock(bestChainHeadBlock))     // maybe delete ****
        {
            if (checkNextBlock(bestChainHeadBlock, transactionPool))
                addBlock(bestChainHeadBlock, transactionPool);
        }
        ...
        if (myLatestBlockChainInfo.isBestChain()) {
            ...
            while (!blockCollection.empty()) {
                addBlock(blockCollection.pop(), transactionPool);
            }
        } else {
            ...
            while (!processingBlockChainInfo.isBestChain()) {
                ...
                if (processingBlock.getContent().getTransactions() != null) {
                    for (Transaction transaction : processingBlock.getContent().getTransactions()) {
            ...
            while (processingBlockChainInfo.getNextBlockHash() != null) {
                ...
                addBlock(processingBlock, transactionPool);
